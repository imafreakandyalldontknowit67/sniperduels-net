...
export default function TrustRow() {
  return (
    <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
      {ITEMS.map(it => (
        <div key={it.label} className="rounded-md border border-dark-600 bg-dark-800/40 p-4 text-center">
          <div className="text-2xl font-black text-accent md:text-3xl">{it.value}</div>
          <div className="mt-1 text-xs uppercase tracking-wider text-gray-400">{it.label}</div>
        </div>
      ))}
    </div>
  );
